rule vcf_report:
    ...
    params:
        ...
        js_files = Path(Path(workflow.snakefile).parent, "resources/math.min.js")
    log:
        "logs/igv-report/{batch}.{event}.log"
    conda:
        "../envs/rbt.yaml"
    shell:
        "rbt vcf-report {input.ref} --bams {params.bams} --vcfs {params.bcfs} -f {params.format_field} -i PROB_* -j {params.template} -d {params.max_read_depth} -l {params.js_files} -- {output}"
