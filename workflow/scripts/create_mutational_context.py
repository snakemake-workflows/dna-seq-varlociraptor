...
for bcf_path in snakemake.input.bcfs:
    ...
    for bcf_record in bcf:
        ...
        if len(ref_base) != 1 or len(alt_bases[0]) != 1:
            print(f"Record skipped - No SNV: {variant_chrom}\t{variant_pos}", file=sys.stderr)
            continue
        ...
        if ref_triplet[1] != ref_base:
            print(f"Error: Missmatching reference base: {variant_chrom}\t{variant_pos}\t{ref_base}\t{ref_triplet}", file=sys.stderr)
            exit()
